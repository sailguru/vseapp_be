#coding: utf-8
# SMSC.RU API (smsc.ru) версия 1.5 (03.07.2019)
...
class SMSC
	...
	def _smsc_send_cmd(cmd, arg = "")
		url_orig = (SMSC_HTTPS ? "https" : "http") + "://smsc.ru/sys/#{cmd}" + ".php?login=" + _urlencode(SMSC_LOGIN) + "&psw=" + _urlencode(SMSC_PASSWORD) + "&fmt=1&charset=#{SMSC_CHARSET}&#{arg}"
		url = url_orig.clone
		uri = URI.parse(url)
